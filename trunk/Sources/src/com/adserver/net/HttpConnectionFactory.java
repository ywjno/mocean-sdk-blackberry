 ...
 * @version 1.0 (June 24, 2009) http://www.versatilemonkey.com
 *          <p>
 *          <p>
 *          <p/>
 *          This code is public domain, do with it whatever you wish.
 *          <p/>
 *          This class aims to simplify HTTP requests on the BlackBerry
 *          platform. It could be extended for other uses (socket,
 *          specifically), but that exercise is left up to the reader.
 *          <p>
 *          Use of this class will require signing your application using RIM
 *          supplied signing keys.
 *          <p/>
 *          The BlackBerry platform provides a multitude of different transports
 *          for network access. These include WiFi, BES, BIS, WAP2 and Direct
 *          TCP.
 *          <p/>
 *          Not all transports are available on all devices, carriers or service
 *          plans. Ordinarily an application must determine on its own which
 *          transports are available for a given device, and attempt to connect
 *          via them in order.
 *          <p/>
 *          Sample use:
 *          <p>
 ...
 * </pre>
 *          <p/>
 *          It is possible that an HttpConnection returned by getNextConnection
 *          will fail in a method undetectable before attemping the request.
 *          <p/>
 *          Notes about the various transports from my experience:
 *          <p/>
 *          <b>Wifi:</b>
 *          <p/>
 *          Wifi is the least cost to the user and is also the fasted by orders
 *          of magnitude
 *          <p>
 *          <p>
 *          <b>BES/BIS:</b>
 *          <p/>
 *          - These are largely the same, except BES goes through the user's
 *          corporate network and may be subject to corporate firewalls
 *          <p>
 *          - BES/BIS are generally offered as unlimited use to anyone with a
 *          BlackBerry specific data plan
 *          <p>
 *          - There is usually a limit imposed on the size of files that can be
 *          retrieved (usually 5mb, but can be as low as 100kb)
 *          <p>
 *          <p/>
 *          <b>Direct TCP and WAP2:</b>
 *          <p>
 *          - These transports use carrier data plans which are sometimes billed
 *          at the same rate as BES/BIS, but sometimes are billed separately. It
 *          is possible for users to be on unlimited data plans via BES/BIS but
 *          be charged per MB for TCP and WAP2. I have never seen the reverse,
 *          however.
 *          <p>
 *          - Some carriers do not have limits on the file size, others will
 *          timeout if you request a file over their limit (instead of a 413
 *          error or similar)
 *          <p>
 *          - Some carriers have intermediary proxies that will alter the
 *          content of returned files (wrapping them in carrier specific
 *          content, for example)
 *          <p>
 *          <p/>
 *          <p/>
 *          Good luck, I hope this makes networking on BlackBerry easier for
 *          you.
 */
class HttpConnectionFactory implements ConnectionIterator {
	/**
	 ...
	 * 
	 */
	public static final int DEFAULT_TRANSPORT_ORDER[] = { TRANSPORT_WIFI,
			TRANSPORT_DIRECT_TCP, TRANSPORT_WAP2, TRANSPORT_BIS, TRANSPORT_BES };
	...
	public HttpConnectionFactory(String url, String extraParameters) {
		this(url, extraParameters, 0);
	}
	...
	/**
	 * Equivalent to
	 * <code>HttpConnectionFactory( url, null, transportPriority )</code>
	 * 
	 * @see #HttpConnectionFactory(String, String, int[])
	 * @param url
	 *            See {@link #HttpConnectionFactory(String, String, int[])}
	...
	/**
	 * Creates a factory specifying the target http/https url and mask
	 * containing which transports should be allowed (default order)
	 * 
	 * This method converts allowedTransports into an array of transports to use
	 * arranging the included transports in the order specified by
	 * {@link #DEFAULT_TRANSPORT_ORDER} Once the translation is complete it is
	 ...
	 * </code>
	 * <p>
	 * But only the transports matching the input mask are included in the
	 * array.
	 * 
	 * @param url
	 *            See {@link #HttpConnectionFactory(String, String, int[])}
	 ...
	 *            See {@link #HttpConnectionFactory(String, String, int[])}
	 * @param allowedTransports
	 *            A set of transports that should be allowed, for example, to
	 *            set only wifi and BES use: TRANSPORT_WIFI | TRANSPORT_BES
	 */
	public HttpConnectionFactory(String url, String extraParameters,
			int allowedTransports) {
		this(url, extraParameters, transportMaskToArray(allowedTransports));
	}

	/**
	 * Creates a factory specifying the target http/https url and ordered list
	 * of transports to attempt
	 * 
	 * This method constructs an <code>HttpConnectionFactory</code> for the URL
	 * specified, using any extra connection parameters specified in
	 * <code>extraParemeters</code> that will follow the order of transports
	 * specified in <code>transportPriority</code>
	 * <p>
	 * Transports not in <code>transportPriority</code> are not used in any
	 * order. The order of transports attempted will follow the order the are
	 * presented in <code>transportPriority</code>
	 * 
	 * <code>extraParameters</code> are additional parameters you want added to
	 * the connection string, each must be preceded by a semicolon. These are
	 * some useful ones:
	 * <p>
	 ...
	 * <p>
	 * Example: ";ConnectionTimeout=120000;EncryptRequired=true" See
	 * http://www.blackberryforums
	 * .com/developer-forum/113137-undocumented-connector-open-parameters.html
	 * for more info.
	 * 
	 * @param url
	 *            The url to generate the HttpConnection for (only http and
	 *            https)
	 * @param extraParameters
	 *            Extra parameters that will get appended to the end of the
	 *            final url used in
	 * @param transportPriority
	 *            An array of transports in the order they should be attempted
	 */
	public HttpConnectionFactory(String url, String extraParameters,
...
//		}
	...
	/**
	 * Generates an HttpConnection using the next available transport according
	 * to the order specified during factory creation to the url specified in
	 * factory creation. See the class description for details on use.
	 * 
	 * @return An HttpConnection using the next transport configured during
	 *         factory creation
	 * @throws NoMoreTransportsException
	 *             No more transports are available to use
	 */
	public HttpConnection getNextConnection() throws NoMoreTransportsException {
		HttpConnection con;
		boolean isHttpConnectionInitialised = false;
		// HttpConnection con = null;
		int curTransport = 0;

		// while( con == null && curIndex < transports.length ) {
		while (!isHttpConnectionInitialised && curIndex < transports.length) {
			System.out.print("1");
			...
			case TRANSPORT_WIFI:
				...
				try {
					...
					isHttpConnectionInitialised = true;
					return con;
			...
			case TRANSPORT_BIS:
				System.out.print("3c");
				// while( con == null ) {
				while (!isHttpConnectionInitialised) {
					// while( true) {
					try {
		...
		// if( con == null ) {
		if (!isHttpConnectionInitialised) {
	 ...
	 * @throws NoMoreTransportsException
	 * @throws IOException
	 *             throws exceptions generated by
	 */
	private HttpConnection getBisConnection(int index)
	 ...
	 *             throws exceptions generated by
	 */
	private HttpConnection getWap2Connection(int index)
			throws NoMoreTransportsException, IOException {
	 ...
	 *            If not null will be concatenated onto the end of after
	 *            transportExtras1
	 ...
	 */
	private HttpConnection getConnection(String transportExtras1,
	 ...
	 * Called upon first instantiation of the class and upload
	 * 
	 * @param reload

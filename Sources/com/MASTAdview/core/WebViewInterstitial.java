...
//	int marginY = 0;
	...
	public WebViewInterstitial(final MASTAdview adserver) {
		super(STYLE);
		thisScreen = this;
		...
		fullScreenManager = new Manager(USE_ALL_WIDTH | USE_ALL_HEIGHT) {
			...
			protected void sublayout(int width, int height) {
				...
				if (placeButon) {
					switch (adserver.getCloseButtonPosition()) {
		...
		new Thread() {
			public void run() {
				try {
					Thread.sleep(adserver.getShowCloseButtonTime());
		...
		if (adserver.getAutoCloseInterstitialTime() > 0) {
			new Thread() {
				public void run() {
					try {
						Thread.sleep(adserver.getAutoCloseInterstitialTime());
						UiApplication.getUiApplication().invokeLater(new Runnable() {
							public void run() {
								try {
